  ...
  <v-container fluid class="pa-0">
    <div class="pagina-paises usa-escala">
      ...
      <div class="navegacao-filtro">
        <v-btn
          class="botao-round-laranja"
          @click="mostrarFiltro = !mostrarFiltro"
          :aria-pressed="mostrarFiltro"
          :aria-expanded="mostrarFiltro.toString()"
          aria-label="Mostrar ou ocultar filtro de países"
          aria-controls="filtro-paises"
        >
          Filtro
        </v-btn>
      </div>

      <div v-if="mostrarFiltro" id="filtro-paises" class="filtro">
        <v-text-field
          v-model="filtro"
      ...
      <div class="lista-paises rounded-xl px-3">
        <v-list role="list" tag="ul">
          <template v-for="(pais, index) in paisesFiltrados" :key="pais.idPais">
            ...
            <v-divider
              v-if="index < paisesFiltrados.length - 1"
              tag="li"
              role="separator"
              aria-hidden="true"
            />
...
export default {
  ...
  computed: {
    paisesFiltrados() {
      ...
      return f === ""
        ? this.paises
        : this.paises.filter((p) => (p.nome || "").toLowerCase().includes(f));
    },
  },
  mounted() {
  ...
  methods: {
    async carregarPaises() {
      try {
        ...
        this.paises = Array.isArray(response.data)
          ? response.data
